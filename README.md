...
The trait removes ordinary city and population unhappiness plus military and improvement maintenance. Ideological Public Opinion is also ignored: under Community Patch's global-Happiness rules, the already-existing Founding Core state exactly cancels only the current Public Opinion unhappiness; under full Vox Populi, Public Opinion is already outside the local citizen-approval ratio, so no bonus is added. Reusing that state preserves the database type roster required by existing saves, while its +6 capital Food is applied independently by the metabolism ledger. A hidden Community Patch dummy policy zeros universal unit upkeep—including civilian and air units—and supplies the purchase gates. Lua clamps the treasury’s real base building maintenance to zero at load, construction, capture, turn processing, and end turn, with the same policy providing a -100% building fallback. The policy is excluded from policy count, score, policy cost, and ideology checks. The DLL clamps unit upkeep and base building maintenance at zero, so no nominal Gold refund or negative-maintenance income is created. Occupied cities are treated as organs, not discontented populations.
...
- Foreign ideological pressure may still appear in the Culture Overview as information, but its Public Opinion unhappiness cannot lower Chorus empire Happiness, trigger uprisings, or cause city defections.
